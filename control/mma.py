import numpy as np
from control.itercount import Iteration
...
class MMA:
    ...
    def __init__(self, function, variables, responses, tolx=1e-4, move=0.1, maxit=100, xmin=0.0, xmax=1.0, itertag='mma', fn_callback=None, **kwargs):
        self.funbl = function
        ...
        self.iter = Iteration(self.itertag)
    ...
    def response(self):
        ...
        self.epsimin *= np.sqrt(self.m + self.n)

        # Set outer bounds
        if not hasattr(self.xmin, '__len__'):
            self.xmin = self.xmin * np.ones(self.n)
        ...
        if not hasattr(self.xmax, '__len__'):
            self.xmax = self.xmax * np.ones(self.n)
        elif len(self.xmax) == len(self.variables):
            xmaxvals = self.xmax
            self.xmax = np.zeros(self.n)
        ...
        while change > self.tolX and (self.iter.value-self.iter0) < self.maxIt:
            ...
            xval, change = self.mmasub(xval.copy(), np.hstack(f), np.vstack(df))
    ...
    def mmasub(self, xval, g, dg):
        ...
        xmma, ymma, zmma, lam, xsi, eta, mu, zet, s = subsolv(self.epsimin, self.low, self.upp, alfa, beta, P, Q, self.a0, self.a, b, self.c, self.d)
        ...
        printstr = ""
        printstr += "it.: {0: 3d}, ".format(self.iter)
        printstr += "obj.: {0:+.3e}, ".format(g[0])
        # printstr += "ch.: {0:.3e}, ".format(change)
        printstr += "dobjnorm: {0:.3e}, ".format(np.linalg.norm(dg[0, :]))
        # printstr += "KKT norm: {0:.3e},".format(KKT)
        for i in range(self.m):
            printstr += "g{0:d}({2:s}): {1:+.4e} | ".format(i + 1, g[i + 1], self.responses[i+1].tag)
